...
#if defined(NDEBUG)
# error "Transboundarycoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Transboundarycoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Transboundarycoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Transboundarycoin: find block wanted by given orphan block
...
// Transboundarycoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Transboundarycoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Transboundarycoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Transboundarycoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Transboundarycoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Transboundarycoin: track money supply and mint amount info
...
// Transboundarycoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Transboundarycoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Transboundarycoin: compute stake entropy bit for stake modifier
    ...
    // Transboundarycoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Transboundarycoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Transboundarycoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Transboundarycoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Transboundarycoin-loadblk");
